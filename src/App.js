...
function App() {
  ...
  const handleAddTodo = () => {
    ...
    toast.success(` Description "${newDescription}" added successfully`);
    setNewTitle("");
    setNewDescription("");
  };
  ...
  const handleComplete = (index) => {
    ...
    updatedCompletedTodoArr.push(fileteredItem);
    setcompletedTodos(updatedCompletedTodoArr);
    handleDeleteTodo(index);
    localStorage.setItem(
      "completedTodos",
      JSON.stringify(updatedCompletedTodoArr)
    );

    toast.success(`Todo "${todoToComplete.title}" marked as completed!`);
  };
  ...
  const handleUpdateTitleDescription = (value) => {
    setCurrentEditedItem((prev) => {
  ...
  const handleUpdateTodo = () => {
    const oldTodo = allTodos[currentEdit];
                ...
                return (
                  <div className="edit_wrapper" key={index}>
                    ...
                    <textarea
                      ...
                      rows={4}
                      onChange={(e) =>
                        handleUpdateTitleDescription(e.target.value)
                      }
